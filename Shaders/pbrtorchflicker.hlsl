...
{
    // Use a combination of sines for a more natural flicker
...
// Modify ComputePointLight to apply flicker to point lights
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    // Improved ambient: use only the RGB part of diffuseAlbedo, modulate by ambientAccess, and apply gamma correction
    float3 ambientColor = ambientAccess * gAmbientLight.rgb * diffuseAlbedo.rgb;
    // Optionally apply a small boost to ambient to avoid overly dark scenes
    ambientColor = pow(ambientColor, 1.0f / 1.2f); // gamma correct ambient
