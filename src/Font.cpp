...
void DungeonStompApp::DisplayHud() {
	...
	if (enableOnscreenDebug) {
		...
		sprintf_s(junk, "res");
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(lx, y), sc, pad, lbl);
		sprintf_s(junk, "%dx%d", mClientWidth, mClientHeight);
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(vx, y), sc, pad, val);
		y += rh;
		...
		sprintf_s(junk, "name");
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(lx, y), sc, pad, lbl);
		sprintf_s(junk, "%s", mGpuName.c_str());
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(vx, y), sc, pad, val);
		y += rh;

		sprintf_s(junk, "vram");
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(lx, y), sc, pad, lbl);
		sprintf_s(junk, "%d MB", (int)(mGpuDedicatedVRAM / (1024 * 1024)));
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(vx, y), sc, pad, val);
		y += rh;

		sprintf_s(junk, "api");
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(lx, y), sc, pad, lbl);
		sprintf_s(junk, "DirectX 12");
		RenderText(arialFont, charToWChar(junk), XMFLOAT2(vx, y), sc, pad, val);
		y += rh;
	}
}
